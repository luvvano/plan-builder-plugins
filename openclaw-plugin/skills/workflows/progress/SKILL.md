...
INIT=$(node "$GSD_TOOLS_PATH" init progress)
```
...
- `STATE=$(node "$GSD_TOOLS_PATH" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$GSD_TOOLS_PATH" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "$GSD_TOOLS_PATH" progress bar --raw)
```
...
CONTEXT: [check if has_context | - if not]
