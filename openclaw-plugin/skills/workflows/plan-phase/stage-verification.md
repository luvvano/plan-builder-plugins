...
This stage embeds the gsd-plan-checker agent role and verifies plans for a phase.
...
<!-- END AGENT ROLE: gsd-plan-checker -->
...
1. Read all PLAN.md files from the phase directory
2. Follow the gsd-plan-checker role above to verify plans against all quality dimensions:
   - Dimension 1: Requirement Coverage
   - Dimension 2: Task Completeness
   - Dimension 3: Dependency Correctness
   - Dimension 4: Key Links Planned
   ...
   - Dimension 6: Verification Derivation
   - Dimension 7: Context Compliance (if CONTEXT.md exists)
   - Dimension 8: Nyquist Compliance (if nyquist_validation enabled)
3. Use `node "$GSD_TOOLS_PATH" verify plan-structure <plan_path>` for structural validation of each plan
4. Determine overall status based on findings
