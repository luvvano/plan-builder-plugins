...
This stage embeds the gsd-phase-researcher agent role and executes research for a phase.
...
```bash
node /Users/egorproskyrin/.claude/get-shit-done/bin/gsd-tools.cjs websearch "your query" --limit 10
```
...
```bash
INIT=$(node /Users/egorproskyrin/.claude/get-shit-done/bin/gsd-tools.cjs init phase-op "${PHASE}")
```
...
```bash
node /Users/egorproskyrin/.claude/get-shit-done/bin/gsd-tools.cjs commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
...
<!-- END AGENT ROLE: gsd-phase-researcher -->
...
- `phase_number` — Phase number (e.g., "02")
- `phase_req_ids` — Requirement IDs this phase must address
...
1. Read ROADMAP.md, REQUIREMENTS.md, and any existing CONTEXT.md for the phase
2. Follow the gsd-phase-researcher role above to research what is needed to plan this phase well
3. Write the research output to `$PHASE_DIR/$PADDED_PHASE-RESEARCH.md`
4. If `commit_docs` is true:
   ```bash
   node "$GSD_TOOLS_PATH" commit "docs($PADDED_PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
   ```
5. Return structured result: "RESEARCH COMPLETE" with key findings summary
