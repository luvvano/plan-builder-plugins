...
This stage embeds the gsd-planner agent role and executes planning for a phase.
...
- Full init context (phase_dir, phase_number, phase_req_ids, etc.)
- Research output (if research stage was run)
- `commit_docs` — Whether to commit results

## Execution Steps

1. Read phase context from orchestrator (phase_dir, phase_req_ids, research output)
2. Read ROADMAP.md, REQUIREMENTS.md, STATE.md, and RESEARCH.md
3. Follow the gsd-planner role above to decompose the phase into PLAN.md files
4. Write PLAN.md files to the phase directory using the Write tool (never heredoc)
5. Return structured result: "PLANNING COMPLETE" with plan count and wave structure
...
When called with `<revision_context>` containing checker issues:

## Revision Inputs

- `<revision_context>` — Structured issues from the plan checker
- Existing PLAN.md files in the phase directory

## Revision Steps

1. Parse checker issues from `<revision_context>`
2. Read existing PLAN.md files that have issues
3. Follow the gsd-planner revision_mode instructions to make targeted updates:
   - Add missing elements to tasks (verify, done, files)
   - Add tasks for uncovered requirements
   - Fix dependency issues
   - Split oversized plans
   - Update must_haves derivation
4. Do NOT rewrite plans from scratch — edit specific flagged sections
5. Write updated PLAN.md files using the Write tool
6. Return structured result: "REVISION COMPLETE" with changes made

## Revision Return Format
...
| Plan | Change | Issue Addressed |
|------|--------|-----------------|
| {phase}-01 | Added <verify> to Task 2 | task_completeness |

### Files Updated
- .planning/phases/{phase-dir}/{phase}-01-PLAN.md
```
