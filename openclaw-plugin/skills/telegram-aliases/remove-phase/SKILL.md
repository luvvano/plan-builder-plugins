...
INIT=$(node "$GSD_TOOLS_PATH" init phase-op "${target}")
```
...
**Auto-mode behavior:** Proceed with removal automatically (treat as "y").
</step>
...
```bash
node "$GSD_TOOLS_PATH" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
