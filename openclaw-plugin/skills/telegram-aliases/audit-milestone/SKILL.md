...
## Step 0: Initialize Milestone Context

```bash
INIT=$(node "$GSD_TOOLS_PATH" init milestone-op)
```
...
```bash
CHECKER_MODEL=$(node "$GSD_TOOLS_PATH" resolve-model gsd-integration-checker --raw)
```

## Step 1: Determine Milestone Scope

```bash
node "$GSD_TOOLS_PATH" phases list
```
...
```bash
PHASE_INFO=$(node "$GSD_TOOLS_PATH" find-phase 01 --raw)
...
Spawn an integration checker agent using the embedded role defined in:

@./stage-check.md

Pass this prompt to the integration checker:
...
## Step 5: Check Requirements Coverage (3-Source Cross-Reference)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$GSD_TOOLS_PATH" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
## Step 6: Aggregate into Milestone Audit File
...
Next Up: Complete milestone — archive and tag
...
Next Up: Plan gap closure — create phases to complete milestone
