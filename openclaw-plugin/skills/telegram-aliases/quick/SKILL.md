...
**Auto-mode behavior:** If `$DESCRIPTION` is empty after parsing, error out:
```
ERROR: Task description required.
Usage: /gsd:quick <description>
Example: /gsd:quick Fix broken login redirect
```
...
INIT=$(node "$GSD_TOOLS_PATH" init quick "$DESCRIPTION")
```
...
```bash
node "$GSD_TOOLS_PATH" commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
